...
static void output_handle_geometry(void *data, struct wl_output *output,
    ...
    (void)output; (void)x; (void)y; (void)physical_width; (void)physical_height; (void)subpixel;
...
static void output_handle_done(void *data, struct wl_output *output) {
    ...
    if (st) {
        WAYWAL_LOG_DEBUG("Output %s (%s) geometry resolved: %ux%u @ %u mHz",
                       ...
                       st->description[0] ? st->description : "unknown",
                       st->width, st->height, st->refresh_mhz);
    }
}
...
output_state_t *output_manager_add(output_manager_t *om, struct wl_registry *reg, uint32_t name, uint32_t version) {
    if (!om || !reg) return NULL;
    ...
    WAYWAL_LOG_INFO("Hotplug: registered display output node id %u (active displays: %u)", name, om->count);
    return st;
}

void output_manager_remove(output_manager_t *om, uint32_t name) {
    if (!om || !om->head) return;
    ...
    while (*curr) {
        ...
        if (entry->wl_name == name) {
            *curr = entry->next;
            ...
            WAYWAL_LOG_INFO("Hotplug: display unplugged id %u (%s) (remaining: %u)",
                          name, entry->name[0] ? entry->name : "unnamed", om->count);
...
output_state_t *output_manager_find_by_id(output_manager_t *om, uint32_t name) {
    if (!om) return NULL;
    for (output_state_t *cur = om->head; cur != NULL; cur = cur->next) {
        if (cur->wl_name == name) return cur;
    }
    return NULL;
}
