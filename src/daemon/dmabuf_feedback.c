...
static void feedback_format_table(void *data, struct zwp_linux_dmabuf_feedback_v1 *feedback,
    ...
    g_fb_state.table = (dmabuf_format_table_entry_t *)mmap(
...
static void feedback_tranche_target_device(void *data, struct zwp_linux_dmabuf_feedback_v1 *feedback,
                                           struct wl_array *device) {
    (void)data; (void)feedback; (void)device;
}
...
static void feedback_tranche_formats(void *data, struct zwp_linux_dmabuf_feedback_v1 *feedback,
    ...
    if (!ctx || !g_fb_state.table) return;
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (idx < g_fb_state.num_entries) {
            ...
            if (fmt == DRM_FORMAT_ARGB8888 || fmt == DRM_FORMAT_XRGB8888) {
                if (ctx->preferred_modifier == DRM_FORMAT_MOD_INVALID || g_fb_state.is_scanout_tranche) {
                    ctx->preferred_modifier = mod;
                    WAYWAL_LOG_INFO("Negotiated optimal DMA-BUF scanout modifier: 0x%016lx (format: 0x%08x)",
...
void dmabuf_feedback_init(dmabuf_context_t *ctx) {
    if (!ctx || !ctx->dmabuf_proto) return;
    ...
    if (zwp_linux_dmabuf_v1_get_version(ctx->dmabuf_proto) >= 4) {
        ...
        if (ctx->default_feedback) {
            zwp_linux_dmabuf_feedback_v1_add_listener(ctx->default_feedback, &feedback_listener, ctx);
...
void dmabuf_feedback_destroy(dmabuf_context_t *ctx) {
    if (!ctx || !ctx->default_feedback) return;
