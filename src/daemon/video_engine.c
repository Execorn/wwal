...
static struct wl_buffer *ensure_output_video_buffer(video_engine_t *ve, output_node_t *out,
    ...
    if (!ve || !out || surf_idx >= WAYWAL_VA_SURFACE_POOL_SIZE) return NULL;
    ...
    if (wl_buf) {
        wl_buffer_add_listener(wl_buf, &g_video_buffer_listener, &out->video_buffer_in_use[surf_idx]);
...
static bool video_engine_start_playback(video_engine_t *ve, output_node_t *out) {
    if (!ve || !ve->demuxer) return false;

    int drm_fd = ve->daemon_state->dmabuf_ctx.drm_fd;
    if (!vaapi_decoder_init(&ve->decoder, drm_fd, ve->demuxer->codec,
                            ve->demuxer->width, ve->demuxer->height)) {
    ...
    struct itimerspec its = {
        .it_interval = { 0, 0 },
        .it_value = { .tv_sec = 0, .tv_nsec = 1000000L } /* 1ms */
    };
...
bool video_engine_load_mem(video_engine_t *ve, int memfd, size_t size, output_node_t *out) {
    if (!ve || memfd < 0 || size == 0) return false;
...
void video_engine_dispatch_frame(video_engine_t *ve) {
    ...
    if (vaapi_decoder_decode_packet(&ve->decoder, &pkt, &decoded_surface)) {
        ...
        while (out != NULL) {
            if (out->surface) {
                struct wl_buffer *wl_buf = ensure_output_video_buffer(ve, out, decoded_surface, surf_idx);
                if (wl_buf) {
                    if (out->viewport) {
    ...
    if (ve->demuxer->fps_num > 0) {
        frame_interval_us = (1000000LL * (int64_t)ve->demuxer->fps_den) / (int64_t)ve->demuxer->fps_num;
    }

    if (ve->playback_speed > 0.01f) {
        frame_interval_us = (int64_t)((float)frame_interval_us / ve->playback_speed);
    }
    if (frame_interval_us < 1000LL) frame_interval_us = 1000LL;

    int64_t next_interval_ns = frame_interval_us * 1000LL;

    /* VRR PLL VBlank Pacing: synchronize presentation to upcoming display refresh */
    output_node_t *target_out = ve->target_output ? ve->target_output : (ve->daemon_state ? ve->daemon_state->outputs : NULL);
    if (target_out && target_out->pres_sync.pll.phase_est_ns > 0) {
        ...
        /* Target: wake up ~1.0 ms prior to predicted hardware VBlank */
        int64_t target_fire_ns = next_vblank_ns - 1000000LL;
        if (target_fire_ns > now_ns && (target_fire_ns - now_ns) < (next_interval_ns * 2)) {
...
void video_engine_pause(video_engine_t *ve) {
    if (!ve || ve->state != WAYWAL_VIDEO_STATE_PLAYING) return;
...
void video_engine_resume(video_engine_t *ve) {
    if (!ve || ve->state != WAYWAL_VIDEO_STATE_PAUSED) return;
...
void video_engine_stop(video_engine_t *ve) {
    if (!ve || ve->state == WAYWAL_VIDEO_STATE_STOPPED) return;
