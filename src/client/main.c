...
int main(int argc, char *argv[]) {
    ...
    if (socket_fd < 0) {
        fprintf(stderr, "Error: Unable to connect to wwald (namespace: %s).\n"
    ...
    switch (opts.cmd) {
        ...
        case CLI_CMD_CLEAR: {
            waywal_clear_payload_t payload = {
                .color = { opts.clear_color.r, opts.clear_color.g, opts.clear_color.b, opts.clear_color.a },
        ...
        case CLI_CMD_VIDEO: {
            ...
            if (waywal_ipc_recv(socket_fd, &resp, &attached_fd) && resp.opcode == WAYWAL_RESP_OK) {
                ...
                fprintf(stderr, "Error: Daemon rejected video request (check if video format is supported)\n");
                ret_code = 1;
            }
            if (attached_fd >= 0) close(attached_fd);
            break;
