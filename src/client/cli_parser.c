...
void cli_print_usage(const char *prog) {
    ...
    printf("  video <PATH> [OPTIONS] Load and play hardware-accelerated video wallpaper (VA-API)\n");
    ...
    printf("  --transition-type <TYPE>   none, simple, fade, wipe, grow, outer, wave, noise (default: fade)\n");
    ...
    printf("  --transition-angle <DEG>   Wipe/wave angle in degrees (default: 0)\n");
...
static uint32_t parse_transition_type(const char *str) {
    ...
    if (strcmp(str, "simple") == 0) return 1;
    if (strcmp(str, "fade") == 0) return 2;
    if (strcmp(str, "wipe") == 0) return 3;
    if (strcmp(str, "grow") == 0) return 4;
    if (strcmp(str, "outer") == 0) return 5;
    if (strcmp(str, "wave") == 0) return 6;
    if (strcmp(str, "noise") == 0) return 7;
    return 2;
}
    ...
    if (strcmp(cmd_str, "ping") == 0) {
        ...
        while (optind < argc) {
            ...
            if (strcmp(arg, "--transition-type") == 0 && optind < argc) {
                ...
                if (dur_s > 0.0f) opts->transition_duration_ms = (uint32_t)(dur_s * 1000.0f);
            } else if (strcmp(arg, "--transition-step") == 0 && optind < argc) {
                int step = atoi(argv[optind++]);
                if (step > 0) opts->transition_duration_ms = (uint32_t)(step * 16);
            } else if (strcmp(arg, "--transition-fps") == 0 && optind < argc) {
                int fps = atoi(argv[optind++]);
                if (fps > 0) opts->transition_fps = (uint32_t)fps;
                ...
                sscanf(argv[optind++], "%f,%f", &opts->transition_wave_freq, &opts->transition_wave_amp);
