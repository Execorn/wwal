...
static void test_bezier_accuracy_and_bench(void) {
    ...
    printf("  Bézier Evaluation: %.2f ns/eval (Goal: < 5.0 ns) [%s]\n",
           ns_per_eval, ns_per_eval < 5.0 ? "PASS" : "WARN");
...
static void test_gpu_compute_transitions(void) {
    ...
    assert(dmabuf_bo_allocate(&ctx, &target_bo, width, height, DRM_FORMAT_ARGB8888, DRM_FORMAT_MOD_INVALID));
    assert(dmabuf_bo_allocate(&ctx, &old_bo, width, height, DRM_FORMAT_ARGB8888, DRM_FORMAT_MOD_INVALID));
    assert(dmabuf_bo_allocate(&ctx, &new_bo, width, height, DRM_FORMAT_ARGB8888, DRM_FORMAT_MOD_INVALID));

    waywal_transition_type_t test_transitions[] = {
        WAYWAL_TRANSITION_FADE,
        WAYWAL_TRANSITION_WIPE,
        WAYWAL_TRANSITION_GROW,
        WAYWAL_TRANSITION_OUTER,
        WAYWAL_TRANSITION_WAVE,
        WAYWAL_TRANSITION_NOISE,
    };
    ...
    printf("  GPU Compute (Wave 1080p): %.3f ms/frame (Goal: < 0.25 ms) [%s]\n",
           ms_per_frame, ms_per_frame < 0.50 ? "PASS" : "WARN");
